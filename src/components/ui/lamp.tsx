...
export const LampContainer = ({
    ...
    return (
        <div
            className={cn(
                // 1. Changed min-h-screen to a fixed, shorter height (or remove it entirely if you want it fluid)
                "relative flex min-h-[70vh] flex-col items-center justify-center overflow-hidden w-full rounded-md z-0 pt-24 pb-12",
                className
            )}
        >
            {/* The Lamp Glowing Effects */}
            ...
            {/* 2. REMOVED -translate-y-80 and replaced it with a much tighter margin/transform adjustment if needed */}
            <div className="relative z-50 flex -translate-y-20 flex-col items-center px-5 mt-0">
                {children}
            </div>
        </div>
    );
